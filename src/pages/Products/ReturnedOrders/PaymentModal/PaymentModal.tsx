...
export const PaymentModal = observer(() => {
  ...
  const settlement = useMemo(() => {
    if (!returnedOrdersStore.singleReturnedOrder || !currencyMany?.data) {
      return {
        debt: { uzs: 0, usd: 0 },
        payment: { uzs: 0, usd: 0 },
        order: { uzs: 0, usd: 0 },
  ...
  }, [returnedOrdersStore.singleReturnedOrder, currencyMany, paymentsForm]);
    ...
    <Modal
      ...
      title={
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          <p style={{ margin: 0 }}>
            To&apos;lov {returnedOrdersStore.singlePayment?.client?.fullname}
          </p>
          <p style={{ margin: 0 }}>
            {debts.length > 0 && (
              <>
      ...
      >
        ...
        >
          <Form.Item
            ...
            rules={[{ required: true }]}
            name="userId"
            initialValue={returnedOrdersStore.singlePayment?.client?.id}
          >
            <Select
              showSearch
              placeholder="Mijoz"
              ...
              options={[{
                value: returnedOrdersStore.singlePayment?.client?.id,
                label: `${returnedOrdersStore.singlePayment?.client?.fullname} ${returnedOrdersStore.singlePayment?.client?.phone}`,
              }]}
              allowClear
            />
          </Form.Item>
